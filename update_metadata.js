 ...
 * Run from the knowledge base root directory: node update_metadata.js
 */
...
function updateMetadata(filepath, metadata) {
  ...
  content = content.replace(
    ...
    `demonstrates_technique: "${metadata.demonstrates_technique.replace(/"/g, '\\"')}"`
  );

  // Update objections_handled
  const objStr = metadata.objections_handled.length > 0
    ? '[' + metadata.objections_handled.map(o => `"${o.replace(/"/g, '\\"')}"`).join(', ') + ']'
    : '[]';
  content = content.replace(
    /objections_handled: \[.*?\]/,
    `objections_handled: ${objStr}`
  );

  // Update seller_profile
  content = content.replace(
    /seller_profile: ".*?"/,
    `seller_profile: "${metadata.seller_profile.replace(/"/g, '\\"')}"`
  );
  ...
  content = content.replace(
    /key_success_factor: ".*?"/,
    `key_success_factor: "${metadata.key_success_factor.replace(/"/g, '\\"')}"`
  );
...
for (const [filepath, metadata] of Object.entries(METADATA)) {
  const fullPath = path.join(baseDir, filepath);
  if (fs.existsSync(fullPath)) {
    if (updateMetadata(fullPath, metadata)) updated++;
  } else {
...
console.log(`\n✅ Updated ${updated} files, ${notFound} not found`);
